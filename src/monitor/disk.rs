...
impl DiskMonitor {
    ...
    fn render_at(&self, term: &mut Terminal, x: i32, y: i32, w: usize, h: usize) {
        ...
        term.set_str(x, cy, "Disks", Some(Color::White), true);
        let total_str = format!("{}/{}", format_bytes(total_used), format_bytes(total_size));
        term.set_str(x + w as i32 - total_str.len() as i32, cy, &total_str, Some(Color::DarkGrey), false);
        cy += 1;
        ...
        for disk in self.disks.iter().take(max_disks) {
            ...
            self.draw_disk_row(term, x, cy, w, &disk.mount_point, pct, &size_str, cpu_gradient_color(pct));
            cy += 1;
        }
        ...
        if self.disks.len() > max_disks {
            ...
            term.set_str(x + w as i32 - msg.len() as i32, cy - 1, &msg, Some(Color::DarkGrey), false);
        }
    }

    fn draw_disk_row(&self, term: &mut Terminal, x: i32, y: i32, width: usize, mount: &str, percent: f32, size_str: &str, color: Color) {
        ...
        term.set_str(pos, y, &mount_display, Some(Color::Cyan), false);
        pos += mount_w as i32;
